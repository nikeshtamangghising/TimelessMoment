...
describe('Cart Utils', () => {
  ...
  describe('validateCartItem', () => {
    it('should validate a valid cart item', () => {
      const result = validateCartItem(mockCartItem1)
  ...
  describe('getCartSummary', () => {
    it('should calculate cart summary with shipping', () => {
      const items = [mockCartItem2] // $49.99 total
      const summary = getCartSummary(items)
      
      expect(summary.subtotal).toBe(49.99)
      expect(summary.shipping).toBe(9.99) // Under $50, so shipping applies
      expect(summary.tax).toBeCloseTo(4.25, 2) // 8.5% of subtotal
      expect(summary.total).toBeCloseTo(64.23, 2)
      expect(summary.itemsCount).toBe(1)
      expect(summary.freeShippingRemaining).toBeCloseTo(0.01, 2)
    })

    it('should calculate cart summary with free shipping', () => {
      const items = [mockCartItem1] // $199.98 total
      const summary = getCartSummary(items)
      
      expect(summary.subtotal).toBe(199.98)
      expect(summary.shipping).toBe(0) // Over $50, so free shipping
      expect(summary.tax).toBeCloseTo(16.998, 2) // 8.5% of subtotal
      expect(summary.total).toBeCloseTo(216.978, 2)
  ...
  describe('removeOutOfStockItems', () => {
    it('should remove inactive products', () => {
      ...
      const filtered = removeOutOfStockItems(items)
  ...
  describe('updateCartItemQuantities', () => {
    it('should limit quantities to available inventory', () => {
      ...
      const updated = updateCartItemQuantities(items)
